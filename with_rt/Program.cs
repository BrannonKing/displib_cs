...
class Program {
    ...
    BuildCpModel(Problem problem, int maxGap) {
        ...
        var outerStarts = new List<List<GRBVar>>(problem.Trains.Count);

        for (int t = 0; t < problem.Trains.Count; t++) {
            var train = problem.Trains[t];
            var starts = new List<GRBVar>(train.Count);
        ...
        Parallel.ForEach(tToChains.Values, new ParallelOptions{MaxDegreeOfParallelism = 8}, chains => {
        ...
        for (int t = 0; t < Math.Min(maxConsToAdd, consToAdd.Count); t++) {
            var (_, c1, c2, cname) = consToAdd[t];
            model.AddConstr(c1, cname + "_a");
            model.AddConstr(c2, cname + "_b");
        }

        Console.WriteLine($"Added up to {maxConsToAdd} of {consToAdd.Count} disjunctions.");
    ...
    static Solution BuildAndOptimize(Problem problem, Stopwatch sw, int maxTime, bool verbose) {
        ...
        if (disjunctions.Count > maxConsToAdd) {
